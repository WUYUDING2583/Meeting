...
class MyGroupListScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: [],
            ...
            .then((responseJson) => {
                ...
                if (responseJson.status === 200) {
                    if (responseJson.data != null) {
                        ...
                        this.setState({ data: groupContain, isGetting: false });
                    } else {
        ...
        fetch(URL, opt).then((response) => response.json())
            .then((responseJson) => {
                ...
                if (responseJson.status === 200) {
                    if (responseJson.data != null) {
                        let groupContain = responseJson.data[1].recommends;
                        this.setState({ data: groupContain, refreshState: RefreshState.Idle });
                    } else {
                        this.setState({ isGetting: false });
                    }
                } else {
    ...
    _empty = () => {
        return (
            <View style={styles.container}>
                ...
                <Text style={styles.text}>你还没有加入的群组欸</Text>
